...
class OT2Env(gym.Env):
    ...
    def reset(self, seed=None, goal_position=None):
        ...
        v_abs = 0
        observation = np.concatenate([pipette_coords, goal_coords, [v_abs]]).astype(np.float32) # Removed speed from observation
    ...
    def step(self, action):
        ...
        if self.prev_pos is not None:
            v_abs = np.linalg.norm(pipette_coords - self.prev_pos)
        else:
            v_abs = 0

        #create full observation
        observation = np.concatenate([pipette_coords, self.goal_position, [v_abs]], axis=0).astype(np.float32)
        self.prev_pos = pipette_coords
        ...
        if d_goal < 0.001:
            ...
            if d_goal < 0.001 and self.steps - self.step_2_stop >= self.count:
                ...
                if self.count <= 0:
                    self.reward_at_stop = 250 - 25
                else:
                    self.reward_at_stop = 250 - (self.count * 50)
                reward += self.reward_at_stop
                self.stopped_at_goal = True
                terminated = True
            else:
                self.reward_at_stop = 250 - ((self.count-1) * 50)
                reward += self.reward_at_stop
                terminated = False
        ...
        if d_goal < self.prev_d_goal:  # Progress made
            self.stagnation_step = 0  # Reset the stagnation counter
        else:  # No progress
            self.stagnation_step += 1

        # Update the previous goal distance for the next step
        self.prev_d_goal = d_goal

        # Check if stagnation threshold is exceeded
        if self.stagnation_step > self.stagnation_threshold:
            truncated = True
            info['reason'] = 'stagnation'
        else:
            info['reason'] = 'normal'
