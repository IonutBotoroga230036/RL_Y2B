...
from ot2_env_wrapper_V14 import OT2Env  # Import the new env
...
parser.add_argument("--batch_size", type=int, default=512, help="Batch size for updating policy and value networks")
parser.add_argument("--n_steps", type=int, default=4096, help="Number of steps to run for each update")
parser.add_argument("--n_epochs", type=int, default=32, help="Number of epochs to update policy network")
...
task = Task.init(project_name='Mentor Group S/Group 3',
                 task_name='RL_230036_W_V14_T_V14')
...
run = wandb.init(project="OT2_230036_V9", sync_tensorboard=True)
...
# Function to log the best model as a WandB artifact
def log_model_as_artifact(model_path, artifact_name, run):
    artifact = wandb.Artifact(name=artifact_name, type="model")
    artifact.add_file(model_path)
    run.log_artifact(artifact)
    print(f"Logged model to WandB: {model_path}")
...
for i in range(10):
    ...
    # Save the model at intermediate checkpoints
